...
def string2binary(string, separate_letters=True):
    ...
    if not separate_letters:
        connector = ''
    binary_string = connector.join(format(ord(letter), 'b').rjust(8, '0') for letter in string)
...
def binary2string(string):
    ...
    separated_letters = textwrap.wrap(decoded_binary, 8)
